if global.paused = -1{
	if active = 1{
		...
		if place_meeting(x,y,DippSword) or place_meeting(x,y,gumShoot) or place_meeting(x,y-global.eGravity,crusher) or place_meeting(x,y,Dynamite) or place_meeting(x,y,dippBeam1) or place_meeting(x,y,kibiKickedP){
			active = 0
		...
		if place_meeting(x-5,y,Ground){
			instance_create_depth(x+20,y,0,springR)
			instance_destroy()
		}
		x = x - 5
	}
